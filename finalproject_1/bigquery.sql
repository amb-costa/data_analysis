...
SELECT 
  ...
  clean.member_casual, 
  clean.rideable_type,
  TIMESTAMP_DIFF(clean.ended_at,clean.started_at,minute) AS ride_duration,
  EXTRACT(dayofweek FROM clean.started_at) AS start_day_week, 
  EXTRACT(day FROM clean.started_at) AS start_day_month, 
  EXTRACT(month FROM clean.started_at) AS start_month, 
  EXTRACT(time FROM clean.started_at) AS start_time,
  clean.start_station_id, 
  inicio.clean_name AS start_station_name, 
  inicio.clean_lat AS start_lat, 
  inicio.clean_lng AS start_lng, 
  EXTRACT(dayofweek FROM clean.ended_at) AS end_day_week, 
  EXTRACT(day FROM clean.ended_at) AS end_day_month, 
  EXTRACT(month FROM clean.ended_at) AS end_month, 
  EXTRACT(time FROM clean.ended_at) AS end_time,
...
WHERE (TIMESTAMP_DIFF(clean.ended_at, clean.started_at,minute) BETWEEN 1 AND 1439) AND clean.start_station_id IS NOT NULL AND clean.end_station_id IS NOT NULL
